...
from pooled_webapp.metadata_store import MetadataStore
...
def _resolve_metadata_db_path() -> Path:
    ...
    return APP_ROOT / "state" / "metadata.sqlite3"


METADATA_DB_PATH = _resolve_metadata_db_path()
UPLOADS_ROOT = APP_ROOT / "state" / "uploads"
...
app = FastAPI(title="Interactive pooled CRISPR analysis")
app.add_middleware(
    SessionMiddleware,
...
def _build_steps(req: RunRequest) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    if mode == "pooled":
        pooled_cmd = [
            _resolve_python(),
            "prpcscreen/scripts/run_pooled_volcano_only.py",
            str(req.raw_dir),
            "--output-dir",
            str(output_abs),
        ]
        ...
        steps = [
            {
                "key": "run_pooled_volcano_only",
                "label": "Run pooled volcano pipeline",
                "cmd": pooled_cmd,
                "kind": "pipeline",
            }
        ]
